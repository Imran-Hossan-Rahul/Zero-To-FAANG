...
First Container: Vector:
    ...
    Understanding C++ Vectors: Mechanics, Memory, and Internal Pointers:
        In C++, a vector is fundamentally known as a Sequential Container, which means it organizes and stores data in a strict, linear sequence in the memory. This concept is built upon three core principles. First, it strictly maintains the Order of Insertion, preserving the exact sequence in which data is added without automatically sorting it like an associative container (such as a set or map). Second, it ensures Contiguous Memory, meaning elements are stored strictly side-by-side in the RAM without any gaps. Finally, this contiguous arrangement allows for instantaneous Positional Access, enabling you to fetch any element directly using its index (like vec[2]). You can visualize this as a line for movie tickets: people stand in the exact order they arrive, physically right next to each other, making it easy to point out exactly who is standing in the 3rd or 4th position.

        To manage this data efficiently behind the scenes, a vector operates using three crucial internal pointers. The Start Pointer holds the memory address of the very first element. The Finish Pointer (which determines the current size) does not point to the end of the total capacity; instead, it points to the very next available empty slot immediately following the last inserted element. Finally, the Capacity Pointer marks the absolute boundary of the currently allocated memory block. When you add a new element and there is still available capacity, the vector simply places the new data exactly where the Finish pointer is resting. It then advances the Finish pointer forward by one slot to prepare for the next element, while the Start and Capacity pointers remain completely unchanged.

        However, a critical operation occurs when the vector runs out of space—specifically, when the Finish pointer collides with the Capacity pointer. Because a vector must maintain contiguous memory, it cannot simply expand into the neighboring memory space, as that space might be occupied by other programs. Instead, it triggers a background process called Reallocation. The vector allocates a brand-new memory block in the Heap, typically double the size of the previous one. It then seamlessly copies or moves all existing elements into this new space in their exact original order. After safely transferring the old data, it inserts the newly added element at the end. Finally, the vector deletes (deallocates) the old memory block and updates its Start, Finish, and Capacity pointers to reference this new, larger space. This process is much like a growing family moving into an apartment that is twice as large when they run out of rooms, safely relocating all their belongings and updating their home address in the process.
    ...
    Vector Functions:
        Time Complexity => 0(1):
            ...
            The Missing push_front(), pop_front(), emplace_front(): Memory, Performance, and STL Philosophy in C++ Vectors
            The absence of a push_front() method in a C++ std::vector is deeply rooted in the container's contiguous memory structure and strict performance considerations. In a vector, elements are stored strictly side-by-side in memory. If you were to insert a new element at the very front (index 0), every single existing element would have to be shifted one space to the right to make room. For a vector with a hundred thousand elements, this means moving a hundred thousand items just to add one. This results in an $O(N)$ time complexity, which is incredibly inefficient for frequent operations.In contrast, the front() method is provided because it simply reads the value located at the Start Pointer. It does not manipulate, shift, or allocate any memory; it just looks at the first element. Because this operation is instantaneous and completely independent of the vector's total size, it operates in guaranteed $O(1)$ time, making it perfectly safe and efficient to use.The decision not to name the front-insertion alternative push_front comes down to the core design philosophy of the C++ Standard Template Library (STL). In the STL, function names act as strict performance guarantees. By convention, functions named push_back, push_front, pop_back, or pop_front are only granted to a container if it can execute them in $O(1)$ time. Since a vector fundamentally cannot insert an element at the front without an $O(N)$ memory shift, it is not allowed to use the push_front name.This naming restriction also acts as a vital psychological safeguard. Programmers naturally associate the word push with high-speed operations. If vector::push_front() existed, developers might casually use it inside massive loops, silently bottlenecking their programs without understanding why. Instead, if you absolutely must insert an element at the front, C++ forces you to use vec.insert(vec.begin(), 100). The word insert, along with the requirement to explicitly pass an iterator, serves as a mental alarm. It signals to the programmer that this is a heavier, generalized operation that will cause data shifting and take $O(N)$ time.Ultimately, this deliberate omission forces developers to choose the right data structure for their specific algorithmic needs. By holding back push_front from vectors, C++ sends a clear structural message: if your program requires frequent front insertions, a vector is simply the wrong tool. Instead, you should utilize a container like std::deque (Double Ended Queue) or std::list, which are specifically engineered to support true $O(1)$ push_front() and push_back() operations natively.
